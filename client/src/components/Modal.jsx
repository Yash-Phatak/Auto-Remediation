      ...
      <div className="modalContainer">
        ...
        <form onSubmit={() => { setOpenModal(false); localStorage.setItem('name', val);  localStorage.setItem('gen', gen)}} className='flex flex-col justify-center items-center'>
          <div className='pb-4'>
            ...
            <input type="text" name="name" placeholder="Your Name" value={val} onChange={(e) => { setVal(e.target.value) }} className='h-8 px-2 border'/>
          </div>
          <div className='pb-4 self-start'>
            <label>Gender: </label>
            <select name="gend" value={gen} onChange={(e) => { setGen(e.target.value) }} className='rounded-sm w-32 h-8 pl-2'>
